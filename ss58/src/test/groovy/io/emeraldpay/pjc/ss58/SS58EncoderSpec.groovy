 ...
 */
class SS58EncoderSpec extends Specification {

    def "Doesn't accept null address type"() {
        when:
        SS58Encoder.instance.encode(null, Hex.decodeHex('9053cc32597892cc2cd43ea6e3c0db7a3b4c52e5fe6052762080dbc3e3222c0b'))
        then:
        thrown(IllegalArgumentException)
    }

    def "Doesn't accept null pubkey"() {
        when:
        SS58Encoder.instance.encode(AddressType.Network.SUBSTRATE, null)
        then:
        thrown(IllegalArgumentException)
    }

    def "Doesn't accept short pubkey"() {
        when:
        SS58Encoder.instance.encode(AddressType.Network.SUBSTRATE, Hex.decodeHex('9053cc32597892cc2cd43ea6e3c0db7a3b4c52e5fe6052762080dbc3e3222c'))
        then:
        thrown(IllegalArgumentException)
    }

    def "Doesn't accept long pubkey"() {
        when:
        SS58Encoder.instance.encode(AddressType.Network.SUBSTRATE, Hex.decodeHex('9053cc32597892cc2cd43ea6e3c0db7a3b4c52e5fe6052762080dbc3e3222c0000'))
        then:
        thrown(IllegalArgumentException)
    }

    def "Kusama self-generated"() {
        setup:
        def encoder = SS58Encoder.instance
        expect:
