...
Class `JavaHttpSubscriptionAdapter` implements `SubscriptionAdapter` (as well as standard `PolkadotApi`)
...
Since the `JavaHttpSubscriptionAdapter` implements standard `PolkadotApi` you can make all other calls through the same WebSocket connection:
