...
/**
 * Extrinsic signer and signature verifier. Created with provided SCALE writer for the call type it
 * support, which is used to create a payload for signature.
 *
...
public class ExtrinsicSigner<CALL extends ExtrinsicCall> {
  ...
  protected byte[] getPayload(ExtrinsicContext ctx, CALL call, boolean asList)
   ...
   */
  public Hash512 sign(ExtrinsicContext ctx, CALL call, Schnorrkel.KeyPair key)
      throws SignException {
   ...
   * @throws SignException if invalid key or failed to encode
   */
  public boolean isValid(ExtrinsicContext ctx, CALL call, Extrinsic.Signature signature,
      Address address)
      throws SignException {
    byte[] payload = getPayload(ctx, call, false);
    ...
    try {
      if (Extrinsic.SignatureType.ED25519 == signatureType) {
        ...
        return Schnorrkel.getInstance()
            .verify(signature.getValue().getBytes(), payload,
                new Schnorrkel.PublicKey(address.getPubkey()));
      }
  ...
  private boolean isValidEd25519Signature(byte[] payload, Extrinsic.Signature signature,
      Address address)
  ...
  public static class SignaturePayloadWriter<CALL extends ExtrinsicCall> implements
      ScaleWriter<SignaturePayload<CALL>> {
